"""PayPal 商户回跳链对账层 (从 GPT-Register-Tool paypal_reconciliation.py 移植)。

核心: reconcile_paypal_return 沿白名单回跳链
  pm-redirects.stripe.com -> pay.openai.com -> chatgpt.com(/checkout/verify)
逐跳验证 (URL 结构 + 阶段跳转合法性 + HTTP 状态 + 正文标记),
产出 secret-free 的 conclusive / unknown / failed 证据。

用途: 提链/授权拿到 BA/支付 URL 后, 验证"商户侧是否确认成功",
unknown(requires_reconciliation) 永不自动重试 (避免重复 side-effect)。
"""
...
_SUCCESS_MARKERS = (
    "payment was successful", "payment successful", "plus is active",
    "subscription is active", "you're all set", "you are all set",
)
_FAILURE_MARKERS = (
    "payment failed", "payment was not successful", "card was declined",
    "setup intent failed", "setup_intent_failed",
)
_CANCEL_MARKERS = (
    "payment cancelled", "payment canceled", "checkout cancelled", "checkout canceled",
)
_PROCESSING_MARKERS = (
    "processing payment", "processing your payment",
    "payment is being processed", "please wait while we process",
)

_URL_VALUE_KEYS = frozenset({
    "url", "href", "location", "redirect_url", "return_url",
    "success_return_url", "verification_url",
})
_CONTAINER_KEYS = ("result", "data", "payload", "b_layer", "merchant", "billing", "authorize")
_INPUT_URL_KEYS = (
    "return_url", "returnURL", "returnUrl", "final_redirect_url",
    "redirect_url", "success_return_url", "verification_url",
)
...
class _Evidence:
    ...
    def terminal_outcome(self) -> Optional[PaymentOutcome]:
        for status in (self.response_status, self.redirect_status, self.stripe_return_status):
...
def normalize_return_state(url: str) -> NormalizedReturnState:
    ...
    if success_return_url:
        ...
        if success_return_stage not in {ReturnStage.CHECKOUT_VERIFY, ReturnStage.CHATGPT_LANDING}:
...
def reconcile_paypal_return(
    ...
    *,
    ...
    transport 不得自动跟跳 (本函数始终传 allow_redirects=False, 每跳先校验再请求)。
    """
    if not isinstance(max_hops, int) or isinstance(max_hops, bool) or max_hops < 1:
        return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                            retryable=False, error_stage="input_validation",
                            error_code="invalid_max_hops", reason="max_hops must be a positive integer")
    if not isinstance(timeout, (int, float)) or isinstance(timeout, bool) or timeout <= 0:
        return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                            retryable=False, error_stage="input_validation",
                            error_code="invalid_timeout", reason="timeout must be positive")
    if transport is None or not callable(getattr(transport, "get", None)):
        return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                            retryable=False, error_stage="input_validation",
                            error_code="invalid_transport", reason="transport must provide get()")
    ...
    if not start_url:
        ...
        if terminal is not None:
            return _make_result(
                ReconciliationClassification.CONCLUSIVE, terminal, retryable=False,
                error_stage="merchant_return",
                error_code="remote_payment_cancelled" if terminal is PaymentOutcome.CANCELLED else "remote_payment_failed",
                reason="merchant return reported a terminal payment state",
                evidence=evidence)
        return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                            retryable=False, error_stage="input_validation",
                            error_code="missing_return_url",
                            reason="no merchant return URL was provided", evidence=evidence)
    ...
    try:
        ...
        return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                            retryable=False, error_stage="input_validation",
                            error_code=exc.code, reason=str(exc), evidence=evidence)
    if current_state.stage is ReturnStage.CHATGPT_LANDING:
        return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                            ...
                            final_stage=current_state.stage, evidence=evidence)
    ...
    for index in range(1, max_hops + 1):
        try:
            ...
            return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                                retryable=False, error_stage="redirect_validation",
                                error_code=exc.code, reason=str(exc),
                                final_stage=current_state.stage, hops=hops, evidence=evidence)

        fingerprint = _url_fingerprint(current)
        if fingerprint in seen:
            return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                                retryable=False, error_stage="redirect_chain",
        ...
        try:
            response = transport.get(current, timeout=float(timeout), allow_redirects=False)
        except Exception as exc:
            hops.append(ReconciliationHop(index=index, stage=current_state.stage,
                                          host=current_state.host, status_code=None))
            return _make_result(ReconciliationClassification.UNKNOWN, PaymentOutcome.UNKNOWN,
                                retryable=True, error_stage=current_state.stage.value,
        ...
        if status_code is None:
            hops.append(ReconciliationHop(index=index, stage=current_state.stage,
                                          host=current_state.host, status_code=None))
            return _make_result(ReconciliationClassification.FAILED, PaymentOutcome.UNKNOWN,
                                retryable=False, error_stage=current_state.stage.value,
        ...
        if terminal is not None:
            hops.append(ReconciliationHop(index=index, stage=current_state.stage,
                                          host=current_state.host, status_code=status_code,
        ...
        if status_code in _REDIRECT_CODES:
            ...
            next_url, next_state = _pick_body_or_nested_hop(current, current_state, body)

        if next_url:
            evidence.observe_url(next_state)
            if (current_state.stage is ReturnStage.CHECKOUT_VERIFY
                    and next_state.stage is ReturnStage.CHATGPT_LANDING):
                evidence.left_verify = True
            hops.append(ReconciliationHop(index=index, stage=current_state.stage,
                                          host=current_state.host, status_code=status_code,
                                          response_state=response_state, next_stage=next_state.stage))
            terminal = evidence.terminal_outcome()
            if terminal is not None:
...
def _validate_transition(current: ReturnStage, target: ReturnStage) -> None:
    allowed = {
        ReturnStage.STRIPE_RETURN: {ReturnStage.STRIPE_RETURN, ReturnStage.OPENAI_PAY, ReturnStage.CHECKOUT_VERIFY},
        ReturnStage.OPENAI_PAY: {ReturnStage.OPENAI_PAY, ReturnStage.CHECKOUT_VERIFY},
        ReturnStage.CHECKOUT_VERIFY: {ReturnStage.CHECKOUT_VERIFY, ReturnStage.CHATGPT_LANDING},
        ReturnStage.CHATGPT_LANDING: {ReturnStage.CHATGPT_LANDING},
    }
    if target not in allowed[current]:
        raise ReturnURLValidationError("invalid_stage_transition",
...
def _observe_mapping(source: str | Mapping[str, Any], evidence: _Evidence) -> None:
    ...
    for current in _known_mappings(source):
        evidence.redirect_status = _merge_status(
            evidence.redirect_status, _normalize_remote_status(current.get("redirect_status")))
        evidence.stripe_return_status = _merge_status(
            evidence.stripe_return_status, _normalize_remote_status(current.get("stripe_return_status")))
...
def _terminal_remote_result(outcome: PaymentOutcome, stage: ReturnStage,
                            hops: Sequence[ReconciliationHop], evidence: _Evidence) -> PayPalReconciliationResult:
    cancelled = outcome is PaymentOutcome.CANCELLED
    return _make_result(ReconciliationClassification.CONCLUSIVE, outcome, retryable=False,
                        error_stage=stage.value,
                        error_code="remote_payment_cancelled" if cancelled else "remote_payment_failed",
                        reason="merchant return reported a terminal payment state",
                        final_stage=stage, hops=hops, evidence=evidence)


def _success_result(stage: ReturnStage, hops: Sequence[ReconciliationHop],
                    evidence: _Evidence) -> PayPalReconciliationResult:
    return _make_result(ReconciliationClassification.CONCLUSIVE, PaymentOutcome.SUCCEEDED,
                        retryable=False, error_stage=None, error_code=None,
                        reason="merchant checkout verification reached a successful terminal state",
                        final_stage=stage, hops=hops, evidence=evidence)


def _make_result(classification: ReconciliationClassification, outcome: PaymentOutcome, *,
                 retryable: bool, error_stage: Optional[str], error_code: Optional[str],
                 reason: str, final_stage: Optional[ReturnStage] = None,
                 hops: Sequence[ReconciliationHop] = (),
                 evidence: Optional[_Evidence] = None) -> PayPalReconciliationResult:
...
class CurlTransport:
    """本项目 transport: 包装 curl_cffi session (chain.make_session / PayPalSession)。"""
...
__all__ = [
    "NormalizedReturnState", "PayPalReconciliationResult", "PaymentOutcome",
    "ReconciliationClassification", "ReconciliationHop", "ReconciliationTransport",
    "RemoteStatus", "ReturnStage", "ReturnURLValidationError", "CurlTransport",
    "normalize_return_state", "reconcile_paypal_return",
]
