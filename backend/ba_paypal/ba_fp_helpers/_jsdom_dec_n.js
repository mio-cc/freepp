...
(async () => {
    ...
    try {
        ...
        const inputN = String(input.decN || '');
        if (inputN) {
            const out = await Promise.race([
                Promise.resolve(win.hsw(0, inputN)).then((x) => String(x)),
                new Promise((_, rej) => setTimeout(() => rej(new Error('dec timeout')), 30000)),
            ]);
            process.stdout.write(JSON.stringify({ ok: true, dec: out.slice(0, 4000), dec_len: out.length, ms: Date.now() - t0 }));
        } else {
