#!/usr/bin/env node
/**
 * 记录 hsw 环境采集器执行时 window/navigator/document API 访问清单 (jsdom)。
 * 用 Proxy 包装 window, 在 aD_ (采集器) 执行期间记录。
 * 使用 wrapped hsw (含 __usCapture), 同时记录 us 加密输出。
 */
'use strict';
const fs = require('fs');
...
// 属性访问记录器: 包装 window 的 getter (在 vm 里无法 Proxy window, 改为记录关键 API 调用)
const reads = {};
    ...
    try {
        ...
        if (desc && desc.get) {
            ...
            Object.defineProperty(win, key, {
                get() { reads['window.' + key] = (reads['window.' + key] || 0) + 1; return orig.call(this); },
                configurable: true,
            });
        }
    } catch (_) {}
...
for (const key of Object.keys(nav)) {
    try {
        if (typeof nav[key] === 'function') continue;
        const desc = Object.getOwnPropertyDescriptor(nav, key);
        if (desc && desc.get) {
            const orig = desc.get;
            Object.defineProperty(nav, key, {
                get() { reads['nav.' + key] = (reads['nav.' + key] || 0) + 1; return orig.call(this); },
                configurable: true,
            });
        }
    } catch (_) {}
...
(async () => {
    ...
    try {
        ...
        const sorted = Object.entries(reads).sort((a, b) => b[1] - a[1]);
        process.stdout.write(JSON.stringify({ ok: true, n_len: n.length, us_len: us.length, reads: sorted, ms: Date.now() - t0 }));
    } catch (e) {
        process.stdout.write(JSON.stringify({ ok: false, error: String(e && (e.stack || e)), n_len: n.length, reads: Object.entries(reads).sort((a, b) => b[1] - a[1]) }));
        process.exit(1);
    }
    process.exit(0);
})();