...
(async () => {
  ...
  for (const p of [
    ...
    path.join(ROOT, 'ba_fp_helpers', 'node_modules'),
    'C:/Users/Administrator/Desktop/GPT_PLUS_PP纯协议版/webui/frontend/node_modules',
  ]) {
    try {
      happy = require(path.join(p, 'happy-dom'));
      break;
    } catch (_) {}
  }
  if (!happy) throw new Error('happy-dom not found');
  const { Browser } = happy;
