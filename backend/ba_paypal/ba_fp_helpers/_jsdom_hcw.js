...
(async () => {
    try {
        ...
        const ex0 = win.__hsw_exports;
        if (ex0 && typeof ex0.hc === 'function') {
            const origHc = ex0.hc.bind(ex0);
            const logs = [];
            ex0.hc = function (a, b, c, d) { logs.push(String(a).slice(0, 400)); return origHc(a, b, c, d); };
            win.__hc_logs = logs;
        }
        ...
        out.hc_logs = (win.__hc_logs || []).slice(0, 6);
