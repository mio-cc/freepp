'use strict';
// jsdom side: hook canvas/audio/perf APIs, run hsw(req), log calls
const fs = require('fs');
...
function hook(obj, name, label) {
  ...
  obj[name] = function () {
    try { log.push([label || name, 'args=' + Array.from(arguments).map(a => { try { return (a && a.constructor && a.constructor.name) || typeof a; } catch (e) { return '?'; } }).join(',')]); } catch (e) {}
...
hook(win.HTMLCanvasElement.prototype, 'getContext', 'getContext');
win.HTMLCanvasElement.prototype.toDataURL = function (type, quality) {
  const cv = createCanvas(this.width || 300, this.height || 150);
  try { log.push(['toDataURL', 'w=' + (this.width || 300) + ' h=' + (this.height || 150) + ' type=' + (type || 'png')]); return cv.toDataURL(type, quality); } catch (e) { return ''; }
};
...
win.HTMLCanvasElement.prototype.getImageData = function (sx, sy, sw, sh) {
  const cv = createCanvas(this.width || 300, this.height || 150);
  try { log.push(['getImageData', sx + ',' + sy + ' ' + sw + 'x' + sh]); return cv.getContext('2d').getImageData(sx, sy, sw, sh); } catch (e) { return null; }
};
...
(async () => {
  ...
  try {
    ...
    process.stdout.write(JSON.stringify({ ok: false, error: String(e).slice(0, 300), log, n_len: n.length }));
    process.exit(1);
  }
  process.stdout.write(JSON.stringify({ ok: true, n_len: n.length, ms: Date.now() - t0, log }));
  process.exit(0);
})();
