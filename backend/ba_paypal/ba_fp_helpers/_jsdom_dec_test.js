#!/usr/bin/env node
/**
 * 最终捕获: 读取已包装的 hsw (由 make_wrapped.js 生成), 跑 hsw(req) 捕获 us。
 * 使用与 _jsdom_mint_unified.js 相同的环境设定。
 */
'use strict';
const fs = require('fs');
...
(async () => {
    ...
    try {
        ...
        n = ''; try { const out = await Promise.race([Promise.resolve(win.hsw(0, new Uint8Array(Buffer.from('DUMMY', 'base64')))), new Promise((_, rej) => setTimeout(() => rej(new Error('timeout')), 30000))]); n = 'dec:' + String(out && out.constructor && out.constructor.name) + ':' + (out ? (out.length || (out.byteLength||0)) : 0); } catch(e) { n = 'decerr:' + String(e && e.message).slice(0,100); }
