...
(async () => {
  ...
  // ─── pack_ext18: 用 hsw(1, encode(body)) 加密 + msgpack 打包 ───
  ...
  const body = {
    n: input.n,
    c: JSON.stringify(input.cObj),
    motionData: {
      st: now - 2500,
      dct: now - 2500,
      mm: [[100, 200, 1], [200, 300, 2], [300, 400, 3]],
      "mm-mp": 0,
      "mm-vs": 0,
      v: 1,
    },
  ...
  const enc = await Promise.race([
    Promise.resolve(w.hsw(1, encode(body))).then(x => Buffer.isBuffer(x) ? x : new Uint8Array(x)),
    new Promise((_, rej) => setTimeout(() => rej(new Error('hsw encrypt timeout')), 60000)),
  ]);
